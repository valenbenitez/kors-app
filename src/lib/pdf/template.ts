...
import { type HeroTag, resolveHeroTags } from "@/lib/pdf/build-hero-tags";
import {
  buildExcludesList,
  buildIncludesList,
  CLIENT_PROVIDED_FLIGHTS_COPY,
  displayExcursionName,
  flightLegFields,
  hasStructuredFlightLeg,
  packageHasFlights,
  textLines,
...
export { CLIENT_PROVIDED_FLIGHTS_COPY };

export type PdfTag = HeroTag;
...
function experienceDetail(exc: { proveedor?: string | null }): string {
...
export function renderPdfHtml(data: PdfRenderData): string {
  ...
  // Prefer seller-edited trip text when present; else auto-build. Fixture
  // overrides (`data.includes` / `data.excludes`) still win for snapshots.
  const includes =
    data.includes ??
    (form.incluyeTexto.trim()
      ? textLines(form.incluyeTexto)
      : buildIncludesList(form));
  const excludes =
    data.excludes ??
    (form.excluyeTexto.trim()
      ? textLines(form.excluyeTexto)
      : buildExcludesList(form));
  // Prefer fixture override; else seller form tags / presets + premium checkbox.
  const tags = data.tags ?? resolveHeroTags(form, totalPax);
  const locationLabel =
    data.locationLabel ??
    (isMulti
      ? form.destinos
